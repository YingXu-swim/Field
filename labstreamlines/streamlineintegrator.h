...
protected:
    ...
    int drawStreamLine(const vec2& startPoint, const VectorField2& vectorField, const float stepSize, const int direction, const bool directionField, const int nSteps,
    ...
    // void DrawStreamLine(const VectorField2& vectorField, const dvec2& position, IndexBufferRAM* indexBufferPoints, IndexBufferRAM* indexBufferLines, std::vector<BasicMesh::Vertex>& vertices);
...
public:
    ...
    TemplateOptionProperty<int> propDirection;// To switch between forward and backward integration
    BoolProperty propnormalizeVectorField;// Normalize the vector field for integration
    // IntProperty propmaxSteps;             // Max number of integration steps
