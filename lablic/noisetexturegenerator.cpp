...
NoiseTextureGenerator::NoiseTextureGenerator()
    ...
    , propseed("propseed", "Rng seed", 1, 1, 100, 1)
    , propblackwhite_("propblackwhite_", "Black white")
...
void NoiseTextureGenerator::process()
{
    ...
    auto outImage = std::make_shared<Image>(size2_t(texSize_.get().x, texSize_.get().y), DataVec4UInt8::get());
    ...
    //LogProcessorInfo("The interpolated color at (0.5,0.5) is " << color << " with grayscale value " << value << ".");
    
    //std::random_device rd;
    //std::mt19937 rng(rd());
    //std::uniform_int_distribution<int> distribution(0, 255);
    
    std::mt19937 rng(propseed.get());                       // 使用 seed 初始化随机数生成器
    std::uniform_int_distribution<int> distribution(0, 255);// 定义取值范围

    for (int j = 0; j < texSize_.get().y; j++)
    {
        for (int i = 0; i < texSize_.get().x; i++)
        {
            val = distribution(rng);// 使用 uniform_int_distribution 生成随机数

            if (propblackwhite_.get())
            {
                val = (val > 127) ? 255 : 0;// 如果大于 127，则设为白，否则设为黑
            }
