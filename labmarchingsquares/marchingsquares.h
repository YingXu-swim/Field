...
protected:
    ...
    vec2 getCoordAtVertex(ivec2 index, const dvec2& bBoxMin, float boundingBoxWidth, float boundingBoxHeight, const ivec2& nVertPerDim) const;
    
    // Get the input value from a 2-dimensional scalar field represented by a 3-dimensional volume
    double getInputValue(const VolumeRAM* data, const size3_t dims, const size_t i, const size_t j);
    ...
    void drawSingleIsoline(float ix, float iy, const double c, const vec4& color, const VolumeRAM* vr, const size3_t dims, IndexBufferRAM* isoBufferGrid, std::vector<BasicMesh::Vertex>& vertices);

    void drawIsolineSingleValue(const double c, const vec4& color, const VolumeRAM* vr, const size3_t dims, IndexBufferRAM* isoBufferGrid, std::vector<BasicMesh::Vertex>& vertices);
...
public:
    ...
    // Output mesh for bounding box andgridlines
...
public:
    ...
    BoolProperty propApplyGaussianFilter{"applyGaussianFilter", "Apply Gaussian Filter", false};
